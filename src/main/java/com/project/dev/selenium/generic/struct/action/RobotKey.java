...
/**
 * TODO: Description of {@code RobotKey}.
 *
 * @author Dyson Parra
...
public class RobotKey extends Action {

    @JsonProperty(value = "vk-key-name")
    protected String vkKeyName;
    ...
    public boolean executeAction(@NonNull WebDriver driver, @NonNull WebElement element, @NonNull By locator, Map<String, String> flagsMap, List<Object> scriptResults) throws Exception {
        vkKeyName = Action.assignScriptResult(vkKeyName, scriptResults);
        eventType = Action.assignScriptResult(eventType, scriptResults);
        RobotEvent event = new RobotEvent();
        if (eventType != null && vkKeyName != null)
            if (eventType.equals("press"))
                event.keyPress(vkKeyName);
            else if (eventType.equals("release"))
                event.keyRelease(vkKeyName);
        return true;
    }
