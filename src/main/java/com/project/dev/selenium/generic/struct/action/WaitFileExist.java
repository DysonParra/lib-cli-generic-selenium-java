...
/**
 * TODO: Description of {@code WaitFileExist}.
 *
 * @author Dyson Parra
...
public class WaitFileExist extends Action {

    @JsonProperty(value = "filepath")
    protected String filePath;
    @JsonProperty(value = "validation-gap")
    protected Integer validationGap;
    ...
    public boolean executeAction(@NonNull WebDriver driver, @NonNull WebElement element, @NonNull By locator, Map<String, String> flagsMap, List<Object> scriptResults) throws Exception {
        filePath = Action.assignScriptResult(filePath, scriptResults);
        int elapsedTime = 0;
        timeout = Integer.valueOf(Action.assignScriptResult(timeout, scriptResults));
        while(!FileProcessor.validateFile(filePath) && elapsedTime <= timeout) {
          if (validationGap < timeout) {
              elapsedTime += validationGap;
              Thread.sleep(validationGap);
          } else {
